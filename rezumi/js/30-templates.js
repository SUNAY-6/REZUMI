...
const ResumeTemplates = {
    ...
    _header(data, styles = {}) {
        ...
        const contactItems = [];
        if (p.email) contactItems.push(`<a href="mailto:${p.email}" style="color:inherit;text-decoration:none;">${p.email}</a>`);
        if (p.phone) contactItems.push(`<a href="tel:${p.phone.replace(/[^\d+]/g, '')}" style="color:inherit;text-decoration:none;">${p.phone}</a>`);
        if (p.address) contactItems.push(`<span>${p.address}</span>`);
        if (p.linkedin) contactItems.push(`<a href="${p.linkedin.startsWith('http') ? p.linkedin : 'https://' + p.linkedin}" target="_blank" rel="noopener" style="color:inherit;text-decoration:none;">LinkedIn</a>`);
        if (p.github) contactItems.push(`<a href="${p.github.startsWith('http') ? p.github : 'https://' + p.github}" target="_blank" rel="noopener" style="color:inherit;text-decoration:none;">GitHub</a>`);
        if (p.portfolio) contactItems.push(`<a href="${p.portfolio.startsWith('http') ? p.portfolio : 'https://' + p.portfolio}" target="_blank" rel="noopener" style="color:inherit;text-decoration:none;">Portfolio</a>`);
        // Custom heading links
        if (p.customLinks) {
            p.customLinks.forEach(cl => {
                if (cl.heading && cl.url) {
                    const href = cl.url.startsWith('http') ? cl.url : 'https://' + cl.url;
                    contactItems.push(`<a href="${href}" target="_blank" rel="noopener" style="color:inherit;text-decoration:none;">${cl.heading}</a>`);
                }
            });
        }
    ...
    // Resolve section name from custom sections or use default
    _sectionName(data, id, defaultName) {
        if (data.customSections && Array.isArray(data.customSections)) {
            const custom = data.customSections.find(s => s.id === id);
            if (custom) return custom.name || defaultName;
        }
        return defaultName;
    },

    // Check if section should be visible
    _sectionVisible(data, id) {
        if (data.customSections && Array.isArray(data.customSections)) {
            const custom = data.customSections.find(s => s.id === id);
            if (custom && custom.visible === false) return false;
        }
        return true;
    },
    ...
    _sectionOrder(data) {
        if (data.customSections && Array.isArray(data.customSections)) {
            return data.customSections.filter(s => s.visible !== false).map(s => s.id);
        }
        return ['summary', 'experience', 'education', 'skills', 'projects', 'certifications', 'achievements', 'languages'];
    },

    // Render sections in custom order
    _renderSections(data, sectionRenderers) {
        const order = this._sectionOrder(data);
        let html = '';
        order.forEach(id => {
            if (this._sectionVisible(data, id) && sectionRenderers[id]) {
                const title = this._sectionName(data, id, sectionRenderers[id].title);
                const content = sectionRenderers[id].render();
                if (content) {
                    html += this._section(title, content);
                }
            }
        });
        return html;
    ...
    render_modern(data) {
        ...
        // Build sections with custom names
        const sections = {};
        if (h.summary && this._sectionVisible(data, 'summary')) {
            sections.summary = { title: this._sectionName(data, 'summary', 'Summary'), html: `<div style="font-size:11.5px;line-height:1.6;color:#374151;">${h.summary}</div>` };
        }
        if (exp.length && this._sectionVisible(data, 'experience')) {
            sections.experience = { title: this._sectionName(data, 'experience', 'Experience'), html: exp.map(e => `<div style="margin-bottom:14px;"><div style="font-size:13px;font-weight:600;">${e.role}</div><div style="font-size:11px;color:#6b7280;">${e.company}${e.location ? ' · '+e.location : ''}</div><div style="font-size:10px;color:#9ca3af;margin-bottom:3px;">${e.start} – ${e.end}</div>${e.bullets ? `<div style="font-size:11px;color:#374151;line-height:1.6;white-space:pre-line;">${e.bullets}</div>` : ''}</div>`).join('') };
        }
        if (proj.length && this._sectionVisible(data, 'projects')) {
            sections.projects = { title: this._sectionName(data, 'projects', 'Projects'), html: proj.map(p => `<div style="margin-bottom:10px;"><div style="font-size:12px;font-weight:600;">${p.name}</div>${p.tech ? `<div style="font-size:10px;color:#6b7280;">${p.tech}</div>` : ''}${p.desc ? `<div style="font-size:11px;color:#374151;margin-top:2px;">${p.desc}</div>` : ''}</div>`).join('') };
        }
        if (sk.length && this._sectionVisible(data, 'skills')) {
            sections.skills = { title: this._sectionName(data, 'skills', 'Skills'), html: `<div style="display:flex;flex-wrap:wrap;gap:5px;">${sk.map(s => `<span style="padding:3px 8px;background:#e8edf4;border-radius:3px;font-size:10px;color:#1e3a5f;">${s}</span>`).join('')}</div>` };
        }
        if (edu.length && this._sectionVisible(data, 'education')) {
            sections.education = { title: this._sectionName(data, 'education', 'Education'), html: edu.map(e => `<div style="margin-bottom:10px;"><div style="font-size:11px;font-weight:600;">${e.degree}${e.course ? ' – '+e.course : ''}</div><div style="font-size:10px;color:#6b7280;">${e.institute}</div>${e.cgpa ? `<div style="font-size:10px;color:#6b7280;">CGPA: ${e.cgpa}</div>` : ''}<div style="font-size:9px;color:#9ca3af;">${e.start} – ${e.end}</div></div>`).join('') };
        }
        if (certs.length && this._sectionVisible(data, 'certifications')) {
            sections.certifications = { title: this._sectionName(data, 'certifications', 'Certifications'), html: certs.map(c => `<div style="margin-bottom:6px;"><div style="font-size:10px;font-weight:600;">${c.name}</div><div style="font-size:9px;color:#6b7280;">${c.org}</div></div>`).join('') };
        }
        if (langs.length && this._sectionVisible(data, 'languages')) {
            sections.languages = { title: this._sectionName(data, 'languages', 'Languages'), html: langs.map(l => `<div style="font-size:10px;margin-bottom:3px;">${l.name} <span style="color:#6b7280;">(${l.level})</span></div>`).join('') };
        }
        if (data.achievements?.length && this._sectionVisible(data, 'achievements')) {
            sections.achievements = { title: this._sectionName(data, 'achievements', 'Achievements'), html: data.achievements.map(a => `<div style="margin-bottom:4px;font-size:10px;">${a.title || ''}</div>`).join('') };
        }

        // Render in custom order
        const order = this._sectionOrder(data);
        const mainSections = ['summary', 'experience', 'projects'];
        const sidebarSections = ['skills', 'education', 'certifications', 'languages', 'achievements'];
        ...
        order.forEach(id => {
            ...
            const rendered = this._section(sec.title, sec.html);
            if (mainSections.includes(id)) mainHTML += rendered;
            else sidebarHTML += rendered;
        });
        ...
        <div style="font-family:'Inter',sans-serif;color:#1a1a1a;">
            ...
            <div style="display:grid;grid-template-columns:1fr 240px;">
                <div style="padding:24px 32px;">${mainHTML}</div>
                <div style="background:#f8f9fc;padding:24px 20px;border-left:1px solid #e5e7eb;">${sidebarHTML}</div>
            </div>
        </div>`;
    },
    ...
    render_minimal(data) {
        ...
        const sn = (id, def) => this._sectionName(data, id, def);
        const sv = (id) => this._sectionVisible(data, id);
        return `
