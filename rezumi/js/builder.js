...
// Sample data for template previews in Quick Resume builder
function getBuilderSampleData() {
    const profiles = Storage.getProfiles();
    const p = profiles.length > 0 ? profiles[0] : {
        ...
        summary: 'Results-driven Senior Software Engineer with 5+ years building high-performance web applications. Led teams of 8+ developers, reduced system latency by 45%, and delivered projects serving 2M+ users. Expert in React, Node.js, and cloud architecture.'
    };
    
    const edu = Storage.getEducation();
    const sampleEdu = edu.length > 0 ? edu.slice(0, 2) : [
        { degree: 'M.Tech', course: 'Computer Science', institute: 'IIT Bombay', cgpa: '9.1', startYear: '2016', endYear: '2018' },
        { degree: 'B.Tech', course: 'Information Technology', institute: 'VIT Vellore', cgpa: '8.7', startYear: '2012', endYear: '2016' }
    ];
    
    const exp = Storage.getExperience();
    const sampleExp = exp.length > 0 ? exp.slice(0, 3) : [
        { role: 'Senior Software Engineer', company: 'Razorpay', startDate: '2022-03', current: true, location: 'Bangalore', responsibilities: '• Led payment gateway migration serving 50M+ transactions monthly\n• Reduced API latency by 45% through query optimization\n• Mentored team of 6 engineers\n• Architected microservices handling 10K req/sec', techUsed: 'React, Node.js, AWS, Kubernetes' },
        { role: 'Software Engineer', company: 'Zomato', startDate: '2019-06', endDate: '2022-02', location: 'Gurgaon', responsibilities: '• Built real-time order tracking system for 1M+ daily orders\n• Implemented A/B testing framework increasing conversions by 22%', techUsed: 'Python, Django, PostgreSQL, Redis' }
    ];
    
    const proj = Storage.getProjects();
    const sampleProj = proj.length > 0 ? proj.slice(0, 3) : [
        { name: 'CloudSync Platform', description: 'Real-time collaboration tool with live cursors and document editing for 500+ concurrent users', techStack: 'React, Socket.io, MongoDB, AWS' },
        { name: 'AI Code Reviewer', description: 'ML-powered code review bot that detects bugs and suggests improvements with 92% accuracy', techStack: 'Python, TensorFlow, GitHub API' }
    ];
    
    const skills = Storage.getSkills();
    const sampleSkills = skills.length > 0 ? skills.slice(0, 12) : ['JavaScript', 'TypeScript', 'React', 'Node.js', 'Python', 'AWS', 'Docker', 'Kubernetes', 'PostgreSQL', 'MongoDB', 'Redis', 'GraphQL'];
    
    const certs = Storage.getCertifications();
    const sampleCerts = certs.length > 0 ? certs.slice(0, 2) : [
        { name: 'AWS Solutions Architect Professional', org: 'Amazon Web Services', date: '2023-08' },
        { name: 'Google Cloud Professional Developer', org: 'Google Cloud', date: '2022-11' }
    ];
    
    // Add a sample photo placeholder for photo templates
    const samplePhoto = p.photo || 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="200" height="200"><rect width="200" height="200" fill="#e2e8f0"/><text x="100" y="100" text-anchor="middle" dominant-baseline="middle" fill="#94a3b8" font-family="sans-serif" font-size="14">Photo</text></svg>');
    
    // Ensure profile has photo for photo template previews
    const profileWithPhoto = { ...p, photo: samplePhoto };
    ...
    return {
        ...
        achievements: Storage.getAchievements().slice(0, 3),
        languages: [{ name: 'English', level: 'Fluent' }, { name: 'Hindi', level: 'Native' }, { name: 'Marathi', level: 'Professional' }],
        hobbies: 'Open Source Contributing, Marathon Running, Tech Blogging',
        template: 'modern'
    };
}
                ...
                ${templates.map(t => {
                        ...
                        try {
                            if (typeof ResumeTemplates !== 'undefined') {
                                const sample = getBuilderSampleData();
                                sample.template = t.id;
                                preview = ResumeTemplates.render(t.id, sample);
                            }
                        } catch(e) { preview = '<div style="padding:8px;font-size:5px;">Preview</div>'; }
                        return `
                    <div class="template-select-card ${builderState.templateId === t.id ? 'selected' : ''}" data-category="${t.category}" onclick="selectTemplate('${t.id}')">
                        <div class="template-select-preview" style="height:180px;overflow:hidden;background:white;">
                            <div style="width:100%;height:100%;transform:scale(0.22);transform-origin:top left;overflow:hidden;">
                                ${preview}
                            </div>
                        </div>
