...
function renderResumePreview() {
    ...
    // Always use the latest currentResumeData — never stale references
    const data = currentResumeData;
    if (!data) {
        const resumes = Storage.getResumes();
        if (resumes.length > 0) {
            currentResumeData = JSON.parse(JSON.stringify(resumes[resumes.length - 1]));
        }
        if (!currentResumeData) return;
    }
    
    // Always read template from current data — never use cached/default
    ...
    if (typeof ResumeTemplates !== 'undefined') {
        // Always render with fresh data - this ensures customSections, links, photos are current
        page.innerHTML = ResumeTemplates.render(template, data);
    }
    
    // Update title
    const titleEl = document.getElementById('preview-title');
    if (titleEl) titleEl.textContent = data.name || 'Resume Preview';
}
...
function applyFinalEdits() {
    ...
    // Deep copy to ensure complete sync
    const updatedData = JSON.parse(JSON.stringify(finalEditData));
    
    // Preserve the current template — never reset to default
    if (!updatedData.template && currentResumeData?.template) {
        updatedData.template = currentResumeData.template;
    }
    
    // Save to storage
    if (updatedData.id) {
        Storage.saveResume(updatedData);
    }
    
    // Update current resume data
    currentResumeData = updatedData;
    
    // Force re-render of preview immediately
    renderResumePreview();
    
    // Close editor
    toggleFinalEditor();
    
    showToast('Changes applied and saved!', 'success');
}
...
function openTemplateChanger() {
    ...
    // Generate realistic previews using sample data
    let previewCards = '';
    templates.forEach(t => {
        let preview = '';
        try {
            if (typeof ResumeTemplates !== 'undefined') {
                const sample = JSON.parse(JSON.stringify(currentResumeData));
        ...
        previewCards += `
            <div onclick="applyTemplateChange('${t.id}')" style="cursor:pointer;border:2px solid ${current === t.id ? 'var(--accent)' : 'var(--border-color)'};border-radius:var(--radius-md);overflow:hidden;transition:0.2s;" onmouseenter="this.style.borderColor='var(--accent)'" onmouseleave="this.style.borderColor='${current === t.id ? 'var(--accent)' : 'var(--border-color)'}'">
                <div style="height:140px;background:white;overflow:hidden;">
                    <div style="width:100%;height:100%;transform:scale(0.17);transform-origin:top left;overflow:hidden;">
                        ${preview}
                    </div>
                </div>
    ...
    openModal('Change Template', `
        <div>
            <p style="font-size:12px;color:var(--text-secondary);margin-bottom:16px;">Select a new template. All your resume data will be preserved and the preview will update instantly.</p>
...
function applyTemplateChange(templateId) {
    ...
    // Update template in current data
    currentResumeData.template = templateId;
    
    // PERSIST to storage — this was the missing step
    if (currentResumeData.id) {
        Storage.saveResume(currentResumeData);
    }
    
    // Force re-render preview with the NEW template
    renderResumePreview();
    
    // Also update manual builder if active
    if (typeof manualData !== 'undefined') {
        manualData.template = templateId;
        if (typeof updateManualPreview === 'function') updateManualPreview();
    }
    
    const templateName = typeof ResumeTemplates !== 'undefined' ? ResumeTemplates.get(templateId).name : templateId;
    showToast('Template changed to "' + templateName + '" — preview updated!', 'success');
    closeModal();
}
